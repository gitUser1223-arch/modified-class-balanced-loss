...
def spearman_correlation(x, y): #非线性相关系数 # new add code on 240902
    # 确保 x 和 y 是 PyTorch 张量
    ...
    # 计算 Spearman 相关系数
    n = x.size(0)
    d_squared_sum = torch.sum(d.float() ** 2)
    rho_CT = 1 - (6 * d_squared_sum) / (n * (n ** 2 - 1))
...
class IOULoss(BaseLoss):
    def __init__(self,
                 ...
                 class_effective_num=None, # new add code on 240902
                 ):
        r"""
        Arguments:
        ...
        self.key_fields = ["weights", "anchor", "bbox_normalize", "normalizer_override","pos_cls_target"]#,"pos_cls_target" new add

        if class_weight is not None and class_effective_num is not None:
            class_weight = torch.load(class_weight) # new add code on 240902
            class_effective_num = torch.load(class_effective_num) # new add code on 240902

            self.class_weight = class_weight # new add code on 240902
            self.class_effective_num = class_effective_num # new add code on 240902
        else:
            self.class_weight = class_weight # new add code on 240902
            self.class_effective_num = class_effective_num # new add code on 240902
    ...
    def forward(self, 
                ...
                anchor=None, 
                bbox_normalize=None,
                pos_cls_target=None # new add code on 240902 正样本的期望类别标签
                ):
        """
        Arguments:
        ...
        if anchor is not None:
            ...
            # 加入spearman相关系数构成的损失： # new add code on 240902
            label = pos_cls_target - 1 #因为EOD框架下，0是背景样本，1-1203是前景，为了和class_weight中的class索引对应，需要-1，从0开始

            if self.class_effective_num is not None and label is not None:
                class_effective_nums = self.class_effective_num.clone()
                class_effective_nums = class_effective_nums.to(label.device)
                sample_nums = class_effective_nums[label] 
                iou = 1.0 - iou_loss(input, target, loss_type='iou', reduction='none', weights=None) #第一遍使用的代码：问题出在了loss_type='iou'上，源代码取了对数形式，使用函数时要注意定义
                #iou = 1.0 - iou_loss(input, target, loss_type='linear_iou', reduction='none', weights=None) # 第二遍：应该使用loss_type='linear_iou'
                correlation_between_regloss_and_sample_nums = abs(spearman_correlation(iou, sample_nums)) 
            else:
                correlation_between_regloss_and_sample_nums = 0.0

            # 应用类别权重 # new add code on 240902
            if (self.class_weight is not None) and (label is not None):
                class_weights = self.class_weight.clone()
                class_weights = class_weights.to(label.device)
                sample_weights = class_weights[label] 
                if weights is not None:
                    weights = weights + 1.0 * sample_weights 
                else:
                    weights = sample_weights #1.0 + 

        loss = iou_loss(input, target, loss_type=self.loss_type, reduction='none', weights=weights)

        loss = _reduce(loss, reduction=reduction, normalizer=normalizer) + 1.0 * correlation_between_regloss_and_sample_nums # new add code on 240902
        # 以后跑第一遍代码时可以print一下数字，看看是不是在合理范围内。

        #return _reduce(loss, reduction=reduction, normalizer=normalizer) # original code
        return loss # new add code on 240902
