...
class LvisEvaluator(Evaluator):
    ...
    def eval(self, res_file, res=None):
        ...
        # 获取每个类别的实例数
        category_instance_count = {cat['id']: cat['instance_count'] for cat in self.gts.cats.values()}
        ...
        for itype, class_agnostic in zip(self.iou_types, self.use_cats):
            ...
            # 对每个实例数区间计算AP
            for ap_name, (min_count, max_count) in ap_intervals.items():
                selected_cats = [cat_id for cat_id, count in category_instance_count.items() if
                                 min_count < count <= max_count]
                if selected_cats:
                    # 针对选定的类别重新计算AP
                    lvis_eval.params.catIds = selected_cats
                    lvis_eval.summarize()

                    logger.info(f'eval {itype} results for {ap_name}: {lvis_eval.results}')
                    for k, v in lvis_eval.results.items():
                        metrics[f'{ap_name}.{itype}.{k}'] = v

            # 打印并保存类别AP结果
            all_cls_aps = lvis_eval._check_cls_ap('ap')
            all_cls_ars = lvis_eval._check_cls_ap('ar')
            with open(f'all_cls_{itype}_aps.log', 'w') as f:
                cats_names = [cat['name'] for cat in self.gts.cats.values()]
                for i in range(all_cls_aps.shape[0]):
                    cls_name = cats_names[i]
                    f.write(f'{i + 1} {cls_name} {all_cls_ars[i]:.3f} {all_cls_aps[i]:.3f}\n')
